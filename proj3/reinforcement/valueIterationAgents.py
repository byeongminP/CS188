...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for nextState, prob in transition:
            ...
            res += prob * (reward + self.discount * self.values[nextState])
        return res
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
        "*** YOUR CODE HERE ***"
        pq, pred = util.PriorityQueue(), util.Counter()

        # assign predecessor values
        for state in self.mdp.getStates():
            if not self.mdp.isTerminal(state):
                for action in self.mdp.getPossibleActions(state):
        ...
        for s in self.mdp.getStates():
            if not self.mdp.isTerminal(s):
                qVals = [self.computeQValueFromValues(s, action) for action in self.mdp.getPossibleActions(s)]
                pq.update(s, -1 * abs(max(qVals) - self.values[s]))
