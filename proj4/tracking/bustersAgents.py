    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        ghostPos = [max(dist, key=lambda x : dist[x]) for dist in livingGhostPositionDistributions]
        dist, action = float('inf'), None
        for l in legal:
            curr = min([self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, l), gp) for gp in ghostPos])
            if curr < dist:
                dist, action = curr, l
        return action
